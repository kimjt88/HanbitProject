...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        Log.d("디테일에서 넘어온 위치값 : ",pos);
        String[] temp1 = pos.split(",");
        Double lat = Double.parseDouble(temp1[0]);
        Double lng = Double.parseDouble(temp1[1]);
        // Add a marker in Sydney and move the camera
        LatLng postion = new LatLng(lat, lng);
        mMap.addMarker(new MarkerOptions().position(postion).title("citi holl"));
