...
public class MovieDAO extends SQLiteOpenHelper {
    ...
    public MovieDTO findOne(String key){
        String sql = "SELECT * FROM movie WHERE column = 'key';";
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(sql,null);
        return null;};

    public ArrayList<MovieDTO> findBy(MovieDTO param){
        String sql = "SELECT * FROM movie WHERE column = 'some word';";
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(sql,null);
        return null;};
    ...
    public void update(MovieDTO param){
        String sql = "UPDATE movie SET column1 = 'value', column2 = 'value' WHERE column = 'key';";
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL(sql);
        db.close();
    };
    ...
    public void delete(String key){
        String sql = "DELETE FROM movie WHERE column = 'key';";
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL(sql);
        db.close();
    };
